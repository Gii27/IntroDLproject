...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, filename, hidden_dim, sent_list, tag_list, mode, batch_size=1, dropout1=0, dropout2=0):
    ...
    def fit(self, sent_list: list, tag_list):
        optimizer = optim.SGD(self.parameters(), lr=0.1, weight_decay=1e-4)
        ...
        log_name = "gpu_epoch{}_{}_{}_{}_no_crf_training_log.txt".format(self.epoch_n, self.batch_size, self.drop1, self.drop2)
        ...
        for epoch in range(self.epoch_n):  # again, normally you would NOT do 300 epochs, it is toy data
            ...
            for step in range(total_step):
                ...
                feats = self._get_lstm_features(sentence_in, sent_lens)
                loss = self.get_loss(feats, targets)
    ...
    def get_loss(self, feats, tags):
        feats = feats.view(-1, feats.size(2))
        tags = tags.view(-1)
        mask = (tags > -1).float().cuda()
        # print(mask)
        mask_count = torch.sum(mask).item()
        lse_feats = torch.log(torch.sum(torch.exp(feats), dim=1))
        # print(lse_feats)
        losses = -feats[range(feats.shape[0]), tags] + lse_feats
        # print(losses)
        losses = losses * mask
        loss = torch.sum(losses) / mask_count
        return loss
    #
    ...
    def predict(self, sent_list: list, batch_size=64) -> list:
        self.eval()
        tag_list = []
        total_step = math.ceil(len(sent_list) / batch_size)
        with torch.no_grad():
            for step in range(total_step):
                start_idx = batch_size * step
                end_idx = start_idx + batch_size
                sent = sent_list[start_idx: end_idx]

                sentence_in = [sent_to_idx(ss, self.word_to_idx) for ss in sent]
                lens = [len(s) for s in sent]
                sentence_in = pad_sequence(sentence_in, batch_first=True).cuda()

                tag_indices = self(sentence_in)

                for l, tags in zip(lens, tag_indices):
                    sent_tag = []
                    for i in range(l):
                        sent_tag.append(self.idx_to_tag[tags[i].item()])
    ...
    def _get_lstm_features(self, sentence, lens):
        ...
        lstm_feats = self.log_soft_max(lstm_feats)
    ...
    def forward(self, sent_list):
        lens = torch.tensor([len(s) for s in sent_list]).cuda()
        lstm_feats = self._get_lstm_features(sent_list, lens)
        tag_seq = torch.argmax(lstm_feats, dim=-1)
        return tag_seq
...
test_model = BiLSTM_CRF('data/word_vec.txt', hidden_dim=100, sent_list=train_word, tag_list=train_chunk, mode="word",
                        batch_size=8, dropout1=0, dropout2=0).cuda()
